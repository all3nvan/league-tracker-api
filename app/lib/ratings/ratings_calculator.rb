...
module Ratings
  class RatingsCalculator
    def summoner_ratings
      summoner_id_to_ratings.reduce([]) do |arr, (id, rating)|
    ...
    def summoner_id_to_ratings
      ...
      games_without_unknown_participants.each do |game|
        ...
        winner_ratings = winning_summoner_ids.map do |id|
          unless summoner_id_to_ratings.key?(id)
            summoner_id_to_ratings[id] = Rating.new
          end
          summoner_id_to_ratings[id]
        end
        ...
        new_winner_ratings.each_with_index do |rating, i|
          ...
          summoner_id_to_ratings[summoner_id] = rating
        end
